...
Active implementation tickets should be written so they can be picked up in a
fresh coding session without relying on prior chat context.
...
- `Implementation Notes`: constraints, shared-helper guidance, fixture usage,
  test expectations, and other coding context that should not be rediscovered
  ad hoc
...
- [epic-03-ui-for-implementation-loop](./epic-03-ui-for-implementation-loop/):
  active UI epic for the local implementation-loop operator experience
